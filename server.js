...
// --- In-memory storage ---
const usedTokens = new Map();      // token -> timestamp
const userProgress = new Map();    // userId -> { count, lastClaimDate, lastSessionToken }
...
<html lang="en">
  <head>
    ...
    <!-- GigaPub Ads -->
    <script data-project-id="1872">!function(){var s=document.currentScript,p=s.getAttribute("data-project-id")||"",cdn=["https://ad.gigapub.tech","https://ru-ad.gigapub.tech"],i=0,t,sc;function load(){sc=document.createElement("script");sc.async=true;sc.src=cdn[i]+"/script?id="+p;clearTimeout(t);t=setTimeout(function(){sc.onload=sc.onerror=null;sc.src="";if(++i<cdn.length)load();},15000);sc.onload=function(){clearTimeout(t)};sc.onerror=function(){clearTimeout(t);if(++i<cdn.length)load();};document.head.appendChild(sc);}load();}();</script>
    <style>
      :root {
    ...
    <script>
        ...
        return new Promise((resolve) => {
          database
            ...
            .catch((error) => {
              ...
              if (document.getElementById("minWithdraw")) {
                document.getElementById("minWithdraw").textContent = "50";
              }
              if (document.getElementById("minWithdrawInfo")) {
            ...
            .once("value", (snapshot) => {
              if (snapshot.exists()) {
                ...
                if (!userData.ip || !userData.deviceID) {
                  ...
                  database
                    ...
                    .catch((error) => {
                      console.error("Error updating user IP/device:", error);
                    });
                } else if (
                  userData.ip !== userIP ||
                  userData.deviceID !== deviceID
              ...
              if (document.getElementById("inviteLink")) {
                document.getElementById(
                  "inviteLink"
                ).value = \`https://t.me/share/url?url=\${encodeURIComponent(
                  \`http://t.me/testosteroneeeeebot/money?startbot=\${userData.referralCode}\`
                )}\`;
              }
        ...
        appConfig.surveys.forEach((survey) => {
          ...
          const surveyHTML = \`
            <div class="col-12 mb-3">
              <div class="card survey-card p-3 \${
                isCompleted ? "opacity-75 border-success" : "border-warning"
              }">
                  ...
                  <div class="flex-grow-1">
                    ...
                    <div class="d-flex gap-2">
                      ...
                      <button 
                        ...
                        onclick="verifySurvey(\${survey.id})"
                        \${!isCompleted ? "disabled" : ""}
                      >
      ...
      function verifySurvey(surveyId) {
        ...
        if (userData.completedSurveys && userData.completedSurveys[survey.id]) {
          ...
          showNotification(
            "Info",
            "Please complete the survey first before claiming reward.",
            "info"
          );
        }
      }

      // Start survey completion check
...
app.post('/start', (req,res)=>{
  ...
  const token = sign({userId, i: now, n: nonce});
...
app.post('/claim', (req,res)=>{
  ...
  if(!userId || !sessionToken) return res.json({success:false,message:'missing fields'});

  const data = verify(sessionToken);
  if(!data || data.userId !== userId) return res.json({success:false,message:'invalid token'});

  if(!usedTokens.has(sessionToken)) return res.json({success:false,message:'token expired or already used'});

  const watchedSec = (Date.now() - data.i)/1000;
  if(watchedSec < MIN_WAIT_SEC) return res.json({success:false,message:`You must wait at least ${MIN_WAIT_SEC}s. Waited: ${watchedSec.toFixed(2)}s`});

  const todayKey = new Date().toDateString();
  const userRec = userProgress.get(userId) || {count:0,lastClaimDate:todayKey,lastSessionToken:null};
  if(userRec.lastClaimDate !== todayKey) { userRec.count = 0; userRec.lastClaimDate = todayKey; }
  if(userRec.count >= DAILY_LIMIT) return res.json({success:false,message:'Daily limit reached'});

  // Prevent double claim with same token
  if(userRec.lastSessionToken === sessionToken) return res.json({success:false,message:'Reward already claimed for this session'});

  usedTokens.delete(sessionToken);
  userRec.count += 1;
  userRec.lastSessionToken = sessionToken;
  userProgress.set(userId, userRec);

  res.json({success:true,message:'Reward granted ✅',progressCompleted:userRec.count});
});
